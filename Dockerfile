...
RUN apt-get update && apt-get install -y --no-install-recommends \
    wget curl unzip gnupg ca-certificates git xvfb x11-utils x11-xserver-utils \
...
RUN groupadd -r testuser && useradd -r -g testuser -G audio,video testuser \
    && mkdir -p /home/testuser /tmp/playwright-cache && chown -R testuser:testuser /home/testuser /app /tmp/playwright-cache
...
RUN mkdir -p reports/screenshots reports/videos reports/extent reports/html-report allure-results \
    && chmod -R 755 reports /tmp/playwright-cache \
    && chown -R testuser:testuser reports /tmp/playwright-cache /app

# Copy NGINX config if exists
COPY nginx/default.conf /etc/nginx/conf.d/default.conf
RUN if [ ! -f /etc/nginx/conf.d/default.conf ]; then echo "NGINX config not found, using default"; fi
